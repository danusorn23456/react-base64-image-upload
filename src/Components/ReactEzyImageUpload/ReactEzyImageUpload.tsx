...
import ReactEzyImageUploadContainer, { IReactEzyImageUploadContainerProps } from './ReactEzyImageUploadContainer';
import ReactEzyImageUploadImageBox, { IReactEzyImageUploadImageBoxProps } from './ReactEzyImageUploadBox';
...
export interface IReactEzyImageUploadProps extends Pick<IReactEzyImageUploadContainerProps, "disabled" | "borderless" | "error" | "width" | "maxWidth" | "imageBoxPerRow">, Pick<IReactEzyImageUploadImageBoxProps, "rounded" | "draggable" | "label"> {
  id?: any;
  name?:any;
  label?: any;
...
export default function ReactEzyImageUpload(props: IReactEzyImageUploadProps) {
  ...
  const onDrop = useCallback(async<T extends File>(acceptedFiles: T[], fileRejections: FileRejection[], event: DropEvent) => {
    ...
    for (let index = 0; index < acceptedFiles.length; index++){
      ...
      await new Promise(resolve => {
        ...
        reader.onload = async () => {
          ...
          let aspectRatio = await getAspectRatio(reader.result as string) as number
          result.aspectRatio = aspectRatio

          let _result = await new Promise(async(inner_resolve, inner_reject) => {
            let respone = false;
            if(onEachChange){
    ...
    if (rejectFiles.length) {
      let alertMsg = rejectFiles.map((file,index) => `\n${index+1}) name:${file.name} size:${file.size}`)
      alert(`ไม่สามารถอัพโหลด ${rejectFiles.length} ไฟล์ดังต่อไปนี้ เพราะมีขนาดไฟล์เกิน ${humanFileSize(maxFileSize)} ${alertMsg}`)
    }

  }, [currentValue, maxFileSize, maxFileUpload, fixedSlot, onEachChange, onChange])

  const { getRootProps } = useDropzone({ onDrop, multiple: !fixedSlot, accept: "image/*" })

  
  useEffect(() => {
    setCurrentValue(fixedSlot ? Array.from({ length: maxFileUpload }, () => ({ path: "", lastModified: "", lastModifiedDate: "", name: "", size: "", type: "", webkitRelativePath: "", dataURL: "" })) : [])
  ...
  }, [width, imageBoxPerRow, rows, maxFileSize, borderless, error, draggable, fixedSlot, labels])



  return (
    <ReactEzyImageUploadContainer disabled={isDisabled} ref={containerRef} borderless={borderless} rows={rows} maxWidth={maxWidth} width={width} imageBoxPerRow={imageBoxPerRow} error={error}>
      {
        currentValue.map((item, index) => (
          <ReactEzyImageUploadImageBox 
            ...
            {...(fixedSlot && !item?.dataURL) && {
              input: !item?.dataURL, ...getRootProps({
      ...
      }
      {
        !!(currentValue.length < maxFileUpload && !fixedSlot) && (
          <ReactEzyImageUploadImageBox rounded={rounded} label={label} input {...getRootProps()} />
        )
      }
      {
        showCaption && <ReactEzyImageUploadCaption maxFileSize={maxFileSize} maxFileUpload={maxFileUpload} values={currentValue}/>
      }
