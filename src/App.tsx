...
function App() {
  ...
  const handleChange = (files: ResultFile[]) => {
    console.log(
      files.map((file) => file.dataURL?.slice(0, 64) + "..."),
      "@@send to backend !!!"
    );
  };
    ...
    <div className="App">
      <div className="Component">
        ...
        <ReactEzyImageUpload
          ...
          onEachChange={(file) => handleEachChange(file)}
        ...
        <form>
          <div className="inputControl">
            ...
            <input
              ...
              onChange={(event) =>
                setWidth(
                  ["%", "px", "rem", "em", "vw", "vh"].some((t) =>
                    event.target.value.includes(t)
                  )
                    ? event.target.value
          ...
          <div className="inputControl InputControl-CheckBox">
            <label htmlFor="fixedSlot">fixedSlot</label>
            <input
              type="checkbox"
              name="fixedSlot"
              defaultChecked={fixedSlot}
              onChange={(event) => setFixedSlot(event.target.checked)}
            />
          </div>
          <div className="inputControl">
          ...
          <div className="inputControlX">
            {labels?.map((item, index) => (
              <div key={index} className="inputControl">
                ...
                <input
                  disabled={!(fixedSlot && maxFileUpload)}
                  type="text"
                  style={{ width: 50 }}
                  name={`labels[${index}]`}
                  defaultValue={index}
          ...
          <div className="inputControl InputControl-CheckBox">
            <label htmlFor="draggable">dragable</label>
            <input
              type="checkbox"
              name="draggable"
              defaultChecked={draggable || true}
              onChange={(event) => setDraggable(event.target.checked)}
            />
          </div>
          <div className="inputControl InputControl-CheckBox">
            <label htmlFor="draggable">rounded</label>
            <input
              type="checkbox"
              name="rounded"
              defaultChecked={rounded || false}
              onChange={(event) => setRounded(event.target.checked)}
            />
          </div>
          <div className="inputControl InputControl-CheckBox">
            <label htmlFor="error">showCaption</label>
            <input
              type="checkbox"
              name="showCaption"
              defaultChecked={showCaption || true}
          ...
          <div className="inputControl InputControl-CheckBox">
            ...
            <input
              type="checkbox"
              name="error"
              defaultChecked={error || false}
              onChange={(event) => setError(event.target.checked)}
            />
          </div>
          <div className="inputControl InputControl-CheckBox">
            <label htmlFor="borderless">borderless</label>
            <input
              type="checkbox"
              name="borderless"
              defaultChecked={borderless || false}
              onChange={(event) => setBorderLess(event.target.checked)}
            />
          </div>
          <div className="inputControl">
            <label htmlFor="maxFileUpload">maxFileUpload</label>
            <input
              type="number"
              name="maxFileUpload"
              defaultValue={maxFileUpload || 8}
              onChange={(event) => setMaxFileUpload(event.target.valueAsNumber)}
            />
          </div>
          <div className="inputControl">
            <label htmlFor="imageBoxPerRow">imageBoxPerRow</label>
            <input
              type="number"
              name="imageBoxPerRow"
              defaultValue={imageBoxPerRow || 4}
              onChange={(event) =>
                setImageBoxPerRow(event.target.valueAsNumber)
              }
            />
          </div>
          <div className="inputControl">
            <label htmlFor="imageBoxPerRow">maxFileSize</label>
            <input
              type="number"
              name="imageBoxPerRow"
              defaultValue={maxFileSize || 10485760}
              onChange={(event) => setMaxFileSize(event.target.valueAsNumber)}
            />
          </div>
        </form>
      </div>
